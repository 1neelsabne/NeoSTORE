...
import { postAddress } from "../config/myServices";
...
function AddModal(props) {
	...
	// Function for Submitting Address data to server

	const onSubmit = (data) => {
		if (data) {
			postAddress(props.email, data)
				.then((res) => {
					if (res.data.flg === 1) {
						props.setAddress([...props.address, res.data.dress]);
